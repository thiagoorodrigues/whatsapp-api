...
import { generateAccessLink, uploadToS3 } from "../config/uploadAws";
...
export const saveMessageFiles = async (req: Request, res: Response): Promise<Response> => {
  const { companyId } = req.user;
  const { id } = req.params;
  const senderId = +req.user.id;
  const chatId = +id;
  ...
  medias.forEach(async (item) => {
    ...
    await uploadToS3(item.buffer, item.originalname);
...
export const getUrlLink = (req: Request, res: Response): Response => {

  const data = req.body as { path: string };

  return res.status(200).json(generateAccessLink(data.path ? (data.path) : ""));
}
