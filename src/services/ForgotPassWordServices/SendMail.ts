...
const SendMail = async (email: string, tokenSenha: string) => {
  ...
  const urlSmtp = "smtp.mailersend.net"; // Use a variável de ambiente para o host SMTP
  const userSmtp = "MS_cNZFzQ@southtecnologia.com.br"; // Use a variável de ambiente para o usuário SMTP
  const passwordSmpt = "V2EhTGjlEPyetcrF"; // Use a variável de ambiente para a senha SMTP
  const fromEmail = "contato@southtecnologia.com.br"; // Use a variável de ambiente para o email de origem

  const transporter = nodemailer.createTransport({
    host: urlSmtp,
    port: 587, // Use a variável de ambiente para a porta SMTP
